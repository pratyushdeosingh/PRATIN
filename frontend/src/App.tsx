...
export default function App(){
 const [tab,setTab]=useState<'market'|'risk-ledger'>('market')
 ...
 useEffect(()=>{if(tab==='risk-ledger')refreshLedger()},[tab])
 ...
 return <div className="shell"><aside className="rail"><div className="brand"><span>P</span><div>PRATIN<small>CAPITAL NETWORK</small></div></div><nav><button className={tab==='market'?'active':''} onClick={()=>setTab('market')}><Gauge/>Market pulse</button><button><Radar/>Opportunities</button><button><Landmark/>Capital agents</button><button className={tab==='risk-ledger'?'active':''} onClick={()=>setTab('risk-ledger')}><ShieldCheck/>Risk ledger</button></nav><div className="rail-foot"><span className="live-dot"/> MARKET ONLINE<small>Demo rail • No real funds</small></div></aside>
 <main><header><div><p className="eyebrow">{tab==='risk-ledger'?'DURABLE AUDIT • EXPLAINABLE EVALUATIONS':'CONTINUOUS CLEARING • LIVE DEMO MARKET'}</p><h1>{tab==='risk-ledger'?<>Invoice Risk <em>Ledger.</em></>:<>Capital, intelligently <em>allocated.</em></>}</h1><p className="lede">{tab==='risk-ledger'?'Every invoice risk evaluation is durably logged with factor-level explainability, uncertainty parameters, and policy versions.':'Every verified invoice enters a competitive market where autonomous providers price risk, protect portfolios and race to satisfy the supplier.'}</p></div>{tab==='market'&&<button className="primary" onClick={phase==='settled'?reallocate:run} disabled={phase==='running'}>{phase==='running'?<RotateCcw className="spin"/>:<Bolt/>}{phase==='running'?' Agents are competing':phase==='settled'?' Run next allocation':' Run flagship market'}</button>}</header>
