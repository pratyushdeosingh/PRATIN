...
export type RiskFactorSource='INVOICE_DERIVED'|'HISTORICAL_COUNTERPARTY'|'VERIFICATION_CHECK'|'POLICY_RULE'
export type RiskFactor={label:string;impact:'positive'|'negative'|'neutral';points:number;explanation:string;reason_code?:string;source_category?:RiskFactorSource}
export type RiskDecisionSummary={score:number;band:string;primary_drivers:string[];risk_contributors:string[];top_risk_contributors:RiskFactor[];top_risk_reducers:RiskFactor[];human_readable_explanation:string}
export type RiskSimulationRequest={scenario_name?:string|null;simulate_duplicate?:boolean|null;simulated_on_time_payment_ratio?:number|null;simulated_days_until_due?:number|null;simulated_amount?:number|null;simulated_verification_status?:string|null;simulated_buyer_rating?:number|null;simulated_supplier_history_months?:number|null;simulated_prior_defaults?:number|null}
export type RiskSimulationResult={scenario_name:string;original_score:number;original_band:string;simulated_score:number;simulated_band:string;score_delta:number;explanation:string;modified_factors:RiskFactor[]}
export type VerificationResult={status:string;confidence:number;verified_fields:string[];uncertain_fields:string[];reasons:string[];reason_codes?:string[];duplicate_check?:{duplicate_detected:boolean;matched_invoice_number:string|null;matched_fields:string[];reasons:string[]}|null;gstin_check?:{is_valid_format:boolean;gstin:string|null;state_code:string|null;state_name:string|null;supplier_state:string|null;state_match:boolean|null;pan_entity_code:string|null;pan_entity_type:string|null;supplier_entity_inferred:string|null;entity_match:boolean|null;warnings:string[]}|null;consistency_warnings?:string[];simulation_notice?:string}
export type RiskAssessment={score:number;band:string;confidence:number;factors:RiskFactor[];missing_information:string[];summary?:RiskDecisionSummary|null;policy_version:string}
...
export type FinancingRequirements={minimum_amount:number;max_settlement_hours:number;desired_tenor_days:number;max_total_cost?:number|null;priority?:'BALANCED'|'FASTEST'|'LOWEST_FEE'|'HIGHEST_ADVANCE'}
export type Opportunity={id:string;created_at:string;status:string;invoice:{invoice_number:string;supplier_name:string;buyer_name:string;amount:number};requirements:FinancingRequirements;evaluation?:{verification:VerificationResult;risk:RiskAssessment;provenance:'SERVICE'|'FIXTURE'};match?:{recommended_offer_id:string|null;ranked_offers:RankedOffer[];recommendation_reasons:string[];tradeoffs?:string[];priority?:string;policy_version:string};integration_status:Record<string,IntegrationStatus>}
...
export const api={
 ...
 run:(id:string,reqs?:FinancingRequirements)=>request<Opportunity>(`/api/opportunities/${id}/run-market`,{method:'POST',body:reqs?JSON.stringify(reqs):undefined}),
